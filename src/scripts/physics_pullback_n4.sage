...
def physics_pullback_n4_exact():
    print("Checking Exact Relation for n=4...")
    n = 4
    roots = [0, 1, 2]
    ...
    for t in range(trials):
        # 1. Kinematics
        lambdas, tildes = sample_spinors_from_twistor(n=n, seed=42+t)
        x = [1, 0]
        y = [0, 1]
        ...
        # M = (-1)^(n-1) * <ab>^8 * F(z) / (norm * prod C_k^2)
        # Identify terms
        
        # C_k for k not in roots
        # For n=4, roots=[0,1,2], non-roots=[3]
        def get_C(i):
        ...
        # reconstruct_mhv_from_laplacian uses default neg=(0,1)
    ...
    if consistent:
        print("\nSUCCESS: Exact identity verified for n=4!")
    else:
        print("\nFAILURE: Identity did not match.")

if __name__ == "__main__":
    physics_pullback_n4_exact()
