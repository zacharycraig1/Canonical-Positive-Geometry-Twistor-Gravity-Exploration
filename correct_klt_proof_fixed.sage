...
def hodges_6pt_mhv(twistor):
    ...
    # CRITICAL: x, y must be chosen from the removed rows/cols (0,1,2)
    # to avoid singularities in the kept submatrix (3,4,5).
    # We choose x=0, y=1.
    x, y = 0, 1
    ...
    for i in range(n):
        ...
        # If i is one of the reference spinors (x or y), the formula is singular.
        # But if i is in the removed set (0,1,2), we don't need Phi[i,i].
        # We only strictly need Phi[i,i] for i in rows_to_keep (3,4,5).
        if ix_ang == 0 or iy_ang == 0:
            # Check if we actually need this diagonal element
            # We are keeping rows 3, 4, 5.
            if i in [3, 4, 5]:
    ...
    # EXPERIMENTAL: Return raw reduced determinant
    return det_Phi_red, "ok"
...
def test_correct_klt_proof(n_tests=200, use_moment_curve=True):
    """
    ...
    for test_idx in range(n_tests):
        ...
        if H != 0 and A != 0:
            ...
            ang01 = float(twistor.get_angle(0, 1))
            ang12 = float(twistor.get_angle(1, 2))
            ang20 = float(twistor.get_angle(2, 0))
            
            norm_factor = (ang01 * ang12 * ang20)**2
            neg_factor = ang01**8
            
            # Hypothesis 1: A / H = c * neg_factor / norm_factor
            # H_mod = H * neg_factor / norm_factor
            # ratio_mod = A / H_mod
            
            cand1 = raw_ratio / (neg_factor / norm_factor) if norm_factor != 0 else 0
            
            # Hypothesis 2: A / H = c * ang01^2  (My previous finding)
            cand2 = raw_ratio / (ang01**2) if ang01 != 0 else 0
            
            # Hypothesis 3: A / H = c * (ang01*ang12)
            # Dimension Z^-2 / Z^-6 = Z^4.  ang*ang = Z^4.
            cand3 = raw_ratio / (ang01 * ang12) if ang01*ang12 != 0 else 0
            
            if test_idx < 10:
                log_message(f"Point {test_idx}: Raw A/H={raw_ratio:.4e}")
                log_message(f"  Cand1 (Full Hodges * <01>^8): {cand1:.6f}")
                log_message(f"  Cand2 (Det * <01>^2): {cand2:.6f}")
                log_message(f"  Cand3 (Det * <01><12>): {cand3:.6f}")
...
def main():
    ...
    log_message("\n" + "="*70)
    log_message("TEST: MOMENT-CURVE SAMPLING")
    log_message("="*70)
    success, scale = test_correct_klt_proof(n_tests=50, use_moment_curve=True)
    ...
    if success:
        ...
        if scale != 1:
            log_message(f"Scale factor: {scale}")
