#!/usr/bin/env sage
# =============================================================================
# S3×S3 HODGES PROJECTION
# =============================================================================
# Try projecting S3×S3 invariants (dim=58) to Hodges amplitude
# This is a larger space than S6 (dim=2) and might contain the amplitude
# =============================================================================
...
# =============================================================================
# MATROID AND OS3 (same as before)
# =============================================================================
...
def compute_S3xS3_invariants(C, triples, Vbasis):
    """Compute S3×S3-invariant subspace (stabilizer of (123)|(456) split)."""
    ...
    # S3×S3 generators: (1,2), (2,3), (4,5), (5,6)
    generators = [
    ...
    for t, perm in enumerate(generators):
        ...
        log(f"  S3xS3 gen {t+1}/4: dim = {B.ncols()}")
    
    return [B.column(i) for i in range(B.ncols())]

# =============================================================================
# SPINOR-HELICITY AND HODGES
# =============================================================================
...
def main():
    log("\n" + "="*70)
    log("S3×S3 HODGES PROJECTION")
    log("="*70)
    log("Using S3×S3 invariants (dim=58) instead of S6 (dim=2)")
    log("="*70)
    ...
    try:
        with open(LOG_FILE, 'w') as f:
            f.write(f"[{ts()}] Starting S3xS3 projection\n")
    except:
        pass
    ...
    # Compute S3×S3 invariants
    log("\nComputing S3×S3 invariants...")
    Vinv = compute_S3xS3_invariants(C, triples, Vbasis)
    log(f"  S3×S3 invariants: {len(Vinv)} vectors")
    
    d = len(Vinv)
    
    if d == 0:
        log("[ERROR] No invariants found!")
        return None, {'status': 'error'}
    
    # Generate kinematic samples (need more than d for overdetermined system)
    log("\nGenerating kinematic samples...")
    n_samples = max(200, d * 3)
    samples = []
    hodges_values = []
    ...
    if len(samples) < d:
        log(f"[ERROR] Need at least {d} samples, only have {len(samples)}")
        return None, {'status': 'insufficient_samples'}
    
    # Build evaluation matrix
    log("\nBuilding evaluation matrix...")
    log(f"  This will be {len(samples)} x {d} = {len(samples) * d} evaluations")
    ...
    try:
        ...
        sorted_coeffs = sorted(enumerate(c), key=lambda x: abs(x[1]), reverse=True)
        log(f"  Top 10 coefficients:")
        for idx, val in sorted_coeffs[:10]:
            log(f"    c[{idx}] = {float(val):.6f}")
        ...
        if errors:
            log(f"  Verification: {len(errors)} points")
        ...
        # Report
        log("\n" + "="*70)
        if max_res < 0.01 and (not errors or max(errors) < 0.01):
            log("[SUCCESS] Physical amplitude found!")
            status = "success"
        elif max_res < 1.0:
            log("[PARTIAL] Direction found with moderate residuals")
            status = "partial"
        else:
            log("[FAILED] Large residuals - amplitude not in this space")
            status = "failed"
        log("="*70)
        ...
        result = {
            ...
            'verification_max_error': max(errors) if errors else None,
            'nnz': nnz,
        ...
        try:
            save({'result': result, 'physical_vec': physical_vec, 'coeffs': c}, 
                 's3xs3_hodges_result.sobj')
            log("Saved result to s3xs3_hodges_result.sobj")
        except Exception as e:
            log(f"Save failed: {e}")
        
        try:
            with open("s3xs3_hodges_report.json", 'w') as f:
                json.dump(result, f, indent=2)
            log("Wrote s3xs3_hodges_report.json")
        except Exception as e:
