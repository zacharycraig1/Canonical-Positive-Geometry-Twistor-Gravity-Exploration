...
def test_correct_klt_proof(n_tests=200, use_moment_curve=True):
    ...
    if ratio_matches > 0:
        if len(set(ratios)) == 1:
            ...
            log(f"  Unique ratios: {len(set(ratios))}")
...
def main():
    log("\n" + "="*70)
    log("CORRECT KLT PROOF: GRAVITY = HODGES")
    log("="*70)
    log("Non-circular test: KLT construction vs Hodges")
    log("Using correct KLT momentum kernel formula")
    log("="*70)
    ...
    log("="*70)
    success, scale = test_correct_klt_proof(n_tests=200, use_moment_curve=True)
    ...
    if success:
        ...
        log("[IN PROGRESS] KLT formula may need refinement")
        log("Check mismatches for patterns")
