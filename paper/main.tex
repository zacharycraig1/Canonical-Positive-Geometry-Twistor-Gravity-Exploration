...
\begin{proof}
\textbf{(i)} A 3-rooted forest with roots $R = \{0,1,2\}$ has each root forming its own tree. Roots never connect to each other (that would merge trees), so edges $(0,1), (0,2), (1,2)$ appear in no forest.
...
\begin{proof}
By exhaustive enumeration of all 3-rooted spanning forests on $K_6$ (see \texttt{src/signed\_geometry/} in the verification packet). An internal edge connects two non-root vertices; each forest has $n - |R| = 3$ edges total.
\end{proof}
...
\begin{proof}
By exhaustive enumeration of all 3-rooted spanning forests on $K_6$ (see \texttt{src/signed\_geometry/} in the verification packet). The root assignment for a non-root vertex $v$ is uniquely determined by the connected component containing $v$, which must contain exactly one root. We enumerate all edge subsets of size 3 that form valid forests and classify by root assignment pattern.
\end{proof}
...
\textbf{Sign rule verification} (Theorem~\ref{thm:sign-rule}):
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
$n$ & Forests & Sign Rule Accuracy & Samples \\
\hline
6 & 108 & 100\% (2160/2160 checks) & 20 \\
7 & 1029 & 100\% (20580/20580 checks) & 20 \\
\hline
\end{tabular}
\end{center}

\textbf{Sign split distribution} (Observation~\ref{obs:empirical-split}):
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
$n$ & Modal Split & Mode Frequency & Samples \\
\hline
6 & (54, 54) & 44\% (22/50) & 50 \\
7 & (515, 514) & 30\% (6/20) & 20 \\
\hline
\end{tabular}
\end{center}

\textbf{Note on $n=7$:} The modal split $(515, 514)$ is the closest possible to 50/50 since 1029 is odd. One sample exhibited 597 zero-weight forests due to degenerate kinematics; these are excluded from the split percentage but the sign rule is still verified for all non-zero terms.
...
\begin{itemize}
\item \texttt{tests/test\_oracle\_match.sage}: Independent implementation check of the MTT identity (compares explicit forest enumeration against Sage matrix determinant)
