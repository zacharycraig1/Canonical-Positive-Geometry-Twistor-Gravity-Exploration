...
\label{conj:klt-forest}
The balanced $(3,3)$ signature of the KLT kernel is causally related to the approximately 50/50 sign split in the forest expansion. More generally, for $n$-point amplitudes where the KLT kernel has signature $(p,q)$, the forest expansion should exhibit a sign tendency reflecting this ratio.
\end{conjecture}

\noindent\textbf{Evidence and heuristic argument:}

\begin{enumerate}
\item[(i)] \textbf{Mixed signs are necessary:} Since the KLT bilinear form $M = A^T S \tilde{A}$ has indefinite signature, the form can take either sign for different inputs. This suggests (but does not rigorously prove) that the forest sum computing the same $M$ must have both positive and negative terms.

\item[(ii)] \textbf{Balanced split from combinatorics:} Under a \textit{heuristic} uniform random sign model for edge weights $w_e$, each forest picks 3 edges, and the product of 3 independent $\pm 1$ signs is balanced: $P(+) = P(-) = 1/2$. Numerical simulation confirms the modal split is $(54, 54)$, occurring in 30\% of random trials. Physical kinematics appear to sample a distribution centered on this balanced point.
\end{enumerate}

\begin{remark}
This conjecture is supported by strong numerical evidence but lacks a rigorous algebraic proof connecting the 6-dimensional KLT eigenstructure to the 108-dimensional forest sign structure. Establishing such a connection is an interesting open problem.
\end{remark}
