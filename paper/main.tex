...
\begin{conjecture}[KLT-Forest Correspondence]
The split signature of the KLT kernel is the geometric origin of the approximately 50/50 sign split in the forest expansion. Specifically, for $n$-point amplitudes where the KLT kernel has signature $(p,q)$, the forest expansion exhibits a sign split approaching $p:q$.
\end{conjecture}

\noindent\textbf{Evidence:} Over 50 generic kinematic samples for $n=6$, the mean positive fraction is $0.494 \pm 0.05$, consistent with the $(3,3)$ signature predicting a 50/50 split. The modal split is exactly $(54, 54)$, occurring in 44\% of samples.
...
\begin{theorem}[Uniqueness of Sign Rule]
\label{thm:uniqueness}
Within the forest triangulation, the sign rule $\varepsilon(F) = \text{sign}(\prod w) \times \text{sign}(\prod C^{\deg})$ is the \textbf{unique} assignment such that the sum reproduces the Hodges determinant.
\end{theorem}

\begin{proof}
The Matrix-Tree Theorem gives:
\begin{equation}
\det(\tilde{L}^{(R)}) = \sum_{F} \prod_{e \in E(F)} a_e = \sum_F \left(\prod_e w_e\right) \cdot \left(\prod_v C_v^{\deg(v)}\right)
\end{equation}

Each term is a specific numerical value determined by the kinematics and reference spinors. Its sign is therefore:
\begin{equation}
\varepsilon(F) = \text{sign}\left(\prod_e w_e \cdot \prod_v C_v^{\deg(v)}\right) = \text{sign}(\prod w) \times \text{sign}(\prod C^{\deg})
\end{equation}

There is no freedom in choosing alternative signs---they are determined by the numerical values. Any alternative assignment $\varepsilon'(F) \neq \varepsilon(F)$ would give a different sum, not equal to the Hodges determinant.
\end{proof}

\begin{remark}
Multiplicativity under factorization (Theorem 7.1) is not an additional constraint but a consequence: for $F = F_L \cup F_R$, we have $\text{term}(F) = \text{term}(F_L) \cdot \text{term}(F_R)$, so $\varepsilon(F) = \varepsilon(F_L) \cdot \varepsilon(F_R)$ automatically.
\end{remark}

This theorem shows that the mixed-sign structure is \textbf{intrinsic} to the forest triangulation and cannot be eliminated by any sign redefinition within this framework.
